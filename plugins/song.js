...
Asena.addCommand({pattern: 'song ?(.*)', fromMe: false, desc: 'play song' , dontAddCommandList: true }, async (message, match) => {
	...
	await message.client.sendMessage(message.jid, '```hey dude wait cheyy Abhishek Ser Song Finding..!```' , MessageType.text, { quoted: message.data });
	...
	var img = await Axios.get(`${result.thumb}`, {responseType: 'arraybuffer'})
        ...
        if(!status) return await message.sendMessage('```SONG NOT FOUND```')
